...
function imprimirPedido(pedido, tipoComanda) {
    ...
    let dadosParaImpressao = `--- NOVO PEDIDO | TSÁLÊACH HAMBURGUERIA | ${tipoComanda} ---\n\n`;
        ...
        pedido.itens.forEach(item => {
            ...
            if (item.bebidas && Object.keys(item.bebidas).length > 0) {
                ...
                for (const nomeBebida in item.bebidas) {
                    ...
                    const nomeExibicao = nomeBebida;


                    dadosParaImpressao += `  -> ${nomeExibicao} (${quantidadeBebida}) - R$ ${(precoBebida * quantidadeBebida).toFixed(2).replace('.', ',')}\n`;
                }
            }
