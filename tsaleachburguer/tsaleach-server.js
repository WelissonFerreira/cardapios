...
// 1. IMPORTAÇÕES E CONFIGURAÇÃO (Omitido)
...
    let dadosParaImpressao = `--- NOVO PEDIDO | TSÁLÊACH | ${tipoComanda} ---\n\n`;
    
    // ... (Lógica de cabeçalho, cliente, endereço omitida para brevidade) ...
...
            // ✅ CORREÇÃO APLICADA: Imprime o sabor principal enviado pelo front-end
            if (item.saborPrincipal) {
                dadosParaImpressao += ` - Sabor: ${item.saborPrincipal}\n`;
            }
            
            // ⚠️ O bloco para múltiplos sabores foi removido para evitar confusão.
...
    }
    
    // CORREÇÃO do troco (garante que é um número e imprime)
