...
const clienteId = 'brasasdochef'
const pedidosRef = db.collection('clientes').doc(clienteId).collection('pedidos');

// --- Objeto que contém as duas impressoras.

const Impressoras = {
    impressoraCozinha: 'POS-58-A',
    impressoraEntregador: 'POS-58-B'
}

// -- Objeto que contém a fila de Impressão, variável que contem o controle da fila.
...
function imprimirPedido(pedido, tipoComanda, impressoraDestino, docId) {
    ...
    let dadosParaImpressao = `--- NOVO PEDIDO | BRASAS DO CHEF | ${tipoComanda} ---\n\n`;
    ...
    printer.printDirect({
        ...
        printer: impressoraDestino,
        type: 'RAW',
        success: function(jobID){
        console.log(`[SUCESSO] Pedido ID ${docId} | Comanda '${tipoComanda}' enviada para a impressora: ${impressoraDestino} (ID do trabalho: ${jobID})`);
        resolve(jobID)
    },
        error: function(err){
...
//=============================================================================================
// Função Assíncrona que se conecta ao snapshot do FireBase.
...
async function processarProximoDaFila() {
    ...
    const trabalho = filaDeImpressao.shift()
...
// ... (seu código de importação e configuração)
    for (const nomeDaImpressora of Object.values(Impressoras)) {

        // --- VALIDAÇÃO DA IMPRESSORA NA INICIALIZAÇÃO ---
printer.printDirect({
    data: 'TESTE DE CONEXAO - SERVIDOR [BRASAS DO CHEF] ONLINE\n\n\n\n',
                ...
                novoPedido.impressoraDestino.forEach(destino => {
                    if (destino === 'cozinha') {
                        let trabalho = {
                            ...
                            impressoraDestino: Impressoras.impressoraCozinha,
                            docId: docId
                        }
                    ...
                    if (destino === 'entregador') {
                        let trabalho = {
                            ...
                            impressoraDestino: Impressoras.impressoraEntregador,
                            docId: docId
                        }
                        filaDeImpressao.push(trabalho)
                        itensAdicionadosAFila++;
