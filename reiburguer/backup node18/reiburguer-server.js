...
// IMPORTAÇÃO ESSENCIAL DO SEU NOVO SERVIÇO DE WHATSAPP
const whatsappService = require('./whatsapp'); 

/* Framework Express e body-parser */
...
appExpress.listen(PORT, () => {
    console.log(`[WEBHOOK] Servidor Express rodando na porta ${PORT}`)
})

appExpress.get('/whatsapp-webhook', (req, res) => {
    const VERIFICAR_TOKEN = 'token-secreto-zap'
    ...
    if (token && mode) {
        if (mode === 'subscribe' && token === VERIFICAR_TOKEN) {
            ...
            return res.status(403).send(`[ERRO] 403! Falha na requisição!`)
        }
    }
})

appExpress.post('/whatsapp-webhook', (req, res) => {
    whatsappService.processarMensagemRecebida(req.body)
    res.sendStatus(200);
})
...
.onSnapshot(snapshot => {
    snapshot.docChanges().forEach(change => {
        if (change.type === 'added') {
            ...
            whatsappService.enviarPedidoWhatsApp(novoPedido, docId)
            .then(sucesso => {
                if (sucesso) {
